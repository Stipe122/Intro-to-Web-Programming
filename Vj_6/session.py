...
def get_or_create_session_id(id):
    userId = str(id)
    http_cookies_str = os.environ.get('HTTP_COOKIE', '')
    get_all_cookies_object = cookies.SimpleCookie(http_cookies_str)
    session_id = get_all_cookies_object.get(
        "session_id").value if get_all_cookies_object.get("session_id") else None
    if session_id is None:
        session_id = db.create_session(userId)
...
def add_to_session(id):
    session_id = get_or_create_session_id(id)
